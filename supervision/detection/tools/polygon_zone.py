...
class PolygonZoneAnnotator:
    ...
    def __init__(
        ...
        text_padding: int = 3,
        display_in_zone_count: bool = False,
        opacity: float = 0,
    ):
        self.zone = zone
        self.color = color
    ...
    def annotate(self, scene: np.ndarray, label: Optional[str] = None) -> np.ndarray:
        ...
        # # トータル数描画
        display_text = str(self.zone.current_count)
        # text_size = cv2.getTextSize(
        #     display_text, self.font, self.text_scale, self.text_thickness
        # )[0]
        if self.display_in_zone_count:
            annotated_frame = draw_text(
                scene=annotated_frame,
                text=(display_text if label is None else label),
                text_anchor=self.center,
                # text_anchor=Point(
                #     int(annotated_frame.shape[1] - text_size[0] / 2 - 10),
                #     int(text_size[1] / 2 + 10),
                # ),
